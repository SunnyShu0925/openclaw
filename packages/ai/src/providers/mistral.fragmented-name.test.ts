...
vi.mock("node:crypto", async () => {
  ...
  return {
    ...actual,
    randomUUID: () =>
      mistralMockState.randomUUIDs.shift() ?? actual.randomUUID(),
  };
});

vi.mock("@mistralai/mistralai", async () => {
  const actual = await vi.importActual<typeof import("@mistralai/mistralai")>(
    "@mistralai/mistralai",
  );
  return {
    ...actual,
        ...
        stream: vi.fn(async (payload: unknown, requestOptions: unknown) => {
          ...
          if (mistralMockState.requestThroughHttpClient) {
            ...
            const response = await httpClient?.request(
              new Request("https://api.mistral.ai/chat"),
            );
            if (response && !response.ok) {
              throw Object.assign(
                new Error(`Mistral HTTP ${response.status}`),
                {
...
async function runMistralToolFixture(
  ...
  const parsedChunks = rawChunks.map((chunk) =>
    chunk.map(parseMistralToolCall),
  );
...
describe("Mistral provider — fragmented streamed function names", () => {
  ...
  it("honors an explicit id when a continuation fragment equals another call's full name", async () => {
    ...
    const { toolCalls } = await runMistralToolFixture(
      "response-explicit-id-name-collision",
      [
        [
          {
            id: "call_a",
  ...
  it("treats a repeated stable-id whole name as idempotent, not concatenated", async () => {
    ...
    const { toolCalls } = await runMistralToolFixture(
      "response-repeated-fragment-name",
      [
        [
          {
            id: "call_name",
  ...
  it("keeps an explicit-id name fragment from capturing a later idless same-name call", async () => {
    ...
    const { toolCalls } = await runMistralToolFixture(
      "response-fragment-alias-no-capture",
      [
        [
          {
            id: "call_a",
        ...
        onPayload: (payload) => {
          ...
          if (p.tools) {
            p.tools = p.tools.filter(
              (tool) => tool.function.name === "get_weather",
            );
          }
          return p;
        },
