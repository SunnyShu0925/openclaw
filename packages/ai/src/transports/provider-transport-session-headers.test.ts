...
describe("managed OpenAI Responses session headers at fetch egress", () => {
  ...
  it("forwards session-affinity headers through boundary-aware dispatch for openai-completions", async () => {
    const requests: Request[] = [];
    const captureFetch: typeof fetch = async (input, init) => {
      requests.push(new Request(input, init));
      return new Response(
        JSON.stringify({
          id: "chatcmpl-dispatch-proof",
          object: "chat.completion",
          model: "accounts/fireworks/models/kimi-k2p6",
          choices: [
            {
              index: 0,
              message: { role: "assistant", content: "dispatch-ok" },
              finish_reason: "stop",
            },
          ],
    ...
    const streamFn = createBoundaryAwareStreamFnForModel(model);
    if (!streamFn) {
      throw new Error("No boundary-aware stream for openai-completions");
    }
    const stream = await streamFn(
      model,
      { messages: [{ role: "user", content: "Reply dispatch-ok", timestamp: 1 }] },
      { apiKey: "test-key", sessionId: "dispatch-affinity-456", cacheRetention: "short" },
    );
    await stream.result();

    expect(requests).toHaveLength(1);
    expect(requests[0]?.headers.get("x-session-affinity")).toBe("dispatch-affinity-456");
    expect(requests[0]?.headers.get("x-client-request-id")).toBe("dispatch-affinity-456");
    expect(requests[0]?.headers.get("session_id")).toBe("dispatch-affinity-456");
  });
});
