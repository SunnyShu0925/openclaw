...
export function bootstrapApplication(): ApplicationRuntime {
  ...
  const sessions = createSessionCapability(gateway, agentSelection, {
    ...
    // The raw key may be a configured alias (e.g. agent:work:main) that the
    // Gateway resolves to global; canonicalize via the UI identity helper so
    // the tracker's dedup key matches what record() uses for observer digests.
    onSessionLifecycleReset: (key, agentId) => {
      ...
      void import("../pages/chat/critical-observer-notice.runtime.ts").then((runtime) =>
        runtime.forgetCriticalObserverTracker({
