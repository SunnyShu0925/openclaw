...
`,
  ...
  it("runs dependency policy guards in PR CI preflight", () => {
    ...
    expect(parsedWorkflow.jobs["check-shard"].env.CHECKOUT_BASE_SHA).toBe(
      "${{ (matrix.task == 'guards' || matrix.task == 'npm-lock') && needs.preflight.outputs.diff_base_revision || '' }}",
    );
    expect(checkShardStep.run).toContain(
      "node scripts/report-test-temp-creations.mjs --base refs/remotes/origin/ci-ratchet-base --head HEAD --no-merge-base",
    );
    expect(checkShardStep.run).toContain(
