...
export async function prepareManualRun(
  ...
  return await locked(state, async () => {
    ...
    // persistQueuedCronRunReservations calls skipCronJobsWithoutOwners which
    // handles ownerless rejection with full transactional guards and carries
    // the runId/tracker through the terminal event for manual runs. Direct
    // run() callers (not via enqueueRun) omit the tracker, so supply an
    // internal one to preserve the ownerless disposition signal.
    ...
    if (!reserved) {
      // skipCronJobsWithoutOwners marks tracker.emitted when it handled the
      // rejection. If not marked, the row disappeared or ownership was
      // restored, so fall through to already-running.
