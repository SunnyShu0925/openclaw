    ...
    if (!prepared.cleanup()) {
      ...
      const readFailure =
        "cause" in outcome
          ...
          : "";
      // Report the directory cleanup actually owns. The worker path adopts an outer
      // staging root while prepared.location is an inner child, so dirname(location)
      // can name a directory the recursive removal already deleted before failing on
      // the retained root.
      throw new Error(
        `${readFailure}SQLite snapshot cleanup failed: ${prepared.cleanupRoot ?? path.dirname(prepared.location)}. Check directory permissions and available storage before retrying.`,
