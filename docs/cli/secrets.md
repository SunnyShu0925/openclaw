...
- `--agent <id>`: scope auth profile target discovery and writes to the selected agent store
...
- Targets secret-bearing fields in `openclaw.json` plus auth profile stores. Discovery covers the canonical shared store (Gateway-wide profiles) and the selected agent's local store; duplicate profile IDs resolve to the agent-local candidate, matching runtime's local-overrides-shared precedence. Saved plans carry an `authProfileStore` field (`"shared"` or `"agent"`) so `secrets apply` routes each target to the correct database. Canonical supported surface: [SecretRef Credential Surface](/reference/secretref-credential-surface).
