...
- `timeoutSeconds`: compaction timeout in seconds. The semantics depend on the active context engine's compaction ownership: (a) for an engine that does **not** own compaction (`ownsCompaction: false` or unset — the built-in legacy engine and the documented runtime-delegating plugin pattern whose `compact()` delegates to OpenClaw's native fallback chain), this is the maximum seconds allowed for a **single fallback candidate attempt**, and each candidate gets its own independent window so a stalled candidate cannot erode later candidates' time; (b) for an engine that **owns** its compaction (`ownsCompaction: true`), this is the maximum seconds for the engine's **whole `compact()` operation** as an outer watchdog. Default: `180`.
