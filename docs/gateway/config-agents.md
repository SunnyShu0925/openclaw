...
- `timeoutSeconds`: compaction timeout in seconds. The semantics depend on the active context engine's compaction ownership: (a) for the **trusted built-in legacy engine** (verified by registry identity, never `info.id`), whose `compact()` delegates to OpenClaw's native per-candidate fallback chain, this is the maximum seconds allowed for a **single fallback candidate attempt**, and each candidate gets its own independent window so a stalled candidate cannot erode later candidates' time; (b) for every other engine — an engine that **owns** its compaction (`ownsCompaction: true`) or a custom/runtime-delegating engine without trusted core identity — this is the maximum seconds for the engine's **whole `compact()` operation** as an outer host watchdog. Default: `180`.
